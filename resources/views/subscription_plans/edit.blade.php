...
<head>
   ...
   <title>Allo work infinity - Modifier un plan d’abonnement</title>
        ...
        <div class="container-fluid">
            ...
            <div class="col-sm-12 col-lg-12">
                ...
                <div class="iq-card-header d-flex justify-content-between">
                    ...
                    <h4 class="card-title">Modifier un plan d’abonnement</h4>
                    </div>
                    <div class="col-sm-12 col-md-6 text-right">
                    <a href="{{ route('subscription-plans.index') }}" class="btn btn-secondary">← Retour à la liste</a>
                    </div>
                </div>
                ...
                <div class="iq-card-body">
                    ...
                    if ($featuresValue === null) {
                        $featuresValue = is_array($plan->features)
                            ? implode(', ', $plan->features)
                            : ($plan->features ?? '');
                    }
                    @endphp

                    <form method="POST" action="{{ route('subscription-plans.update', $plan) }}" novalidate>
                    @csrf
                    @method('PUT')
                    ...
                    <form action="{{ route('subscription-plans.destroy', $plan) }}" method="POST" onsubmit="return confirm('Supprimer ce plan ?');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-danger">Supprimer le plan</button>
                    </form>
                </div>
